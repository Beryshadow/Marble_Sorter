...
/* locations des pots en steps à partir limite switch */
#define W_LOC 10   // blanc
#define R_LOC 84   // rouge
#define G_LOC 148  // vert
#define B_LOC 216  // bleue
#define MIDDLE 111 // millieu

/* location de la bille du sensor et du trou en steps à partir limite switch */
#define TROU_LOC 10   // trou
#define SENS_LOC 110  // sensor
#define BILLE_LOC 230 // bille
...
struct Rgb
{
    ...
    int g;
    int b;
    Rgb(int r, int g, int b) : r(r), g(g), b(b){}; // initialisation d'un struct
    void print()
    {
        Serial.print("(R: ");
        Serial.print(r);
...
struct Tcs230
{
    int fs0;       // Settings de freq 1 (frequence scaling number 0)
    int fs1;       // Settings de freq 2 (frequence scaling number 1)
    int cs0;       // setting de couleur 1 (color scaling number 0)
    int cs1;       // setting de couleur 2 (color scaling number 1)
    int out;       // sortie du capteur de couleur
    int led;       // LED
    int threshold; // threshold pour la couleur blanche plus haut = détecte blanc plus souvent
    int readCount; // Nombre de fois qu'on va faire le color sense
    // Setting de frequence 1, 2, couleur 1, 2, sortie, led, threshold, nombre de lecture
    Tcs230(int fs0, int fs1, int cs0, int cs1, int out, int led, int threshold, int readCount) : fs0(fs0), fs1(fs1), cs0(cs0), cs1(cs1), out(out), led(led), threshold(threshold), readCount(readCount){};
    // utiliser dans le void setup
    void sensor_setup()
    {
        pinMode(fs0, OUTPUT);
        pinMode(fs1, OUTPUT);
    ...
    // (LL 0%) (LH 2%) (HL 20%) (HH 100%)
    void frequency_setup(int first, int second)
    {
        digitalWrite(fs0, first);
        digitalWrite(fs1, second);
    }
    Rgb read()
    {
        Rgb rgb(0, 0, 0);       // initialisation du Rgb
        digitalWrite(led, LOW); // allume la led
        for (int i = 0; i < readCount; i++)
        {
            // vert
            digitalWrite(cs0, HIGH);                              // pin pour vert
            digitalWrite(cs1, HIGH);                              // pin pour vert
            int greenfrequency = pulseIn(out, LOW);               // lit la valeur
            greenfrequency = map(greenfrequency, 30, 90, 255, 0); // map la valeur
            rgb.g += greenfrequency;                              // augmente la some
            delay(COLORREAD);                                     // delay
            // rouge
            digitalWrite(cs0, LOW); // set pin pour la couleur rouge

            int bluefrequency = pulseIn(out, LOW);              // lit la valeur
            bluefrequency = map(bluefrequency, 25, 70, 255, 0); // map la valeur
            rgb.b += bluefrequency;                             // augmente la some
            delay(COLORREAD);                                   // delay
            // bleue
            digitalWrite(cs1, LOW);                           // set la pin pour la couleur bleue
            int redfrequency = pulseIn(out, LOW);             // lit la valeur
            redfrequency = map(redfrequency, 25, 72, 255, 0); // map la valeur
            rgb.r += redfrequency;                            // augmente la some
            delay(COLORREAD);                                 // delay
        }

        digitalWrite(led, HIGH); // eteint la led
        rgb.r /= readCount;      // divise la some par le nombre de lecture
        rgb.g /= readCount;      // meme chose ici
        rgb.b /= readCount;      // meme chose ici
        return rgb;              // retourne les valeurs
    }

    // Prend un struct rgb et la converti a un chiffre que goto_color va comprendre
    int rgb_to_step(Rgb couleur)
    {
        int max;                                              // on initialise le max
        int diff_entre_min_max;                               // on initialise la difference entre le max et le avg du rest
        if (couleur.r >= couleur.g && couleur.r >= couleur.b) // check si rouge est max
        {
            max = R_LOC;                                                  // rouge est max
            diff_entre_min_max = couleur.r - (couleur.g + couleur.b) / 2; // calcule la difference entre le max et le avg du rest
        }
        else if (couleur.g >= couleur.b) // check si vert est max
        {
            max = G_LOC;                                                  // vert est max
            diff_entre_min_max = couleur.g - (couleur.r + couleur.b) / 2; // calcule la difference entre le max et le avg du rest
        }
        else // bleue est la seule option restante
        {
            max = B_LOC;                                                  // bleue est max
            diff_entre_min_max = couleur.b - (couleur.r + couleur.g) / 2; // calcule la difference entre le max et le avg du rest
        }
        return (diff_entre_min_max > threshold) ? max : W_LOC; // si la différence est plus grande que le seuil on retourne la couleur sinon on retourne blanc
    }
};
// Moteur avec une limit switch
struct Moteur_s
{
...
void loop()
{
     ...
     {
         while (1) {} // arrête de trier les billes
     }
